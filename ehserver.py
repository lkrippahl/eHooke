...
def process_html(html_source,session_id):
    fil = open(html_source)
    source = fil.read()
    fil.close()
    return string.replace(source,SESSION_ID,session_id)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if 'ID=' in parsed_url.query:
            ...
            html = process_html(HTML_SESSION,new_session_id)
        else:
            fn = HTML_FOLDER+path[1:]
            print 'Name:', fn
    ...
    def do_POST(self):
        ...
        if not res:
            ...
            self.send_header('Location',SERVER_URL+SESSION_PAGE+'?ID='+msg)
